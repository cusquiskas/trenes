...
class mdlVagonDB {
	...
	function set($array) {
		...
		if (isset($array['tx_pasajero'])  ) $this->setTxpasajero  ($array['tx_pasajero']  );
		if (isset($array['id_tren'])      ) $this->setIdTren      ($array['id_tren']      );
		if (isset($array['ix_orden'])     ) $this->setIxOrden     ($array['ix_orden']     );
		if (isset($array['tx_type'])      ) $this->setTxType      ($array['tx_type']      );
		if (isset($array['tx_referencia'])) $this->setTxReferencia($array['tx_referencia']);
	}
	function setIdVagon     ($valor) { $this->id_vagon      = ($valor==null)?null:   (int)$valor; }
	function setTxPasajero  ($valor) { $this->tx_pasajero   = ($valor==null)?null:(string)$valor; }
	function setIdTren      ($valor) { $this->id_tren       = ($valor==null)?null:   (int)$valor; }
	function setIxOrden     ($valor) { $this->ix_orden      = ($valor==null)?null:   (int)$valor; }
	function setTxType      ($valor) { $this->tx_type       = ($valor==null)?null:(string)$valor; }
	function setTxReferencia($valor) { $this->tx_referencia = ($valor==null)?null:(string)$valor; }

	function getIdVagon      () { return $this->id_vagon;      }
	function getTxPasajero   () { return $this->tx_pasajero;   }
	function getIdTren       () { return $this->id_tren;       }
	function getIxOrden      () { return $this->ix_orden;      }
	function getTxType       () { return $this->tx_type;       }
	function getTxReferencia () { return $this->tx_referencia; }
...
class VagonDB extends mdlVagonDB {
	...
	public function listar(){
		...
		if (count($result)==0) $result[] = new mdlVagonDB();
		return $return;
	}
